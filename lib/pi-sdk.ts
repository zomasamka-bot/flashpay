...
export const authenticateCustomer = async (): Promise<{
  ...
  try {
    ...
    if (isStuckPayment) {
      console.warn("[CUSTOMER-AUTH] Stuck payment detected - attempting emergency clear...")
      ...
      try {
        ...
        if (clearResponse.ok) {
          console.log("[CUSTOMER-AUTH] Emergency clear succeeded")
        ...
        console.error("[CUSTOMER-AUTH] Emergency clear failed:", clearError)
...
export const authenticateMerchant = async (): Promise<{
  ...
  try {
    ...
    // Also store the accessToken for later verification
    ...
    // For stuck payment errors, try to clear it via emergency endpoint and then retry
    if (isStuckPayment) {
      console.warn("[MERCHANT-AUTH] Stuck payment detected - attempting emergency clear...")
      CoreLogger.warn("Detected stuck payment error - attempting emergency clear...")
      try {
        const clearResponse = await fetch(`${config.appUrl}/api/emergency/clear-stuck-payment`, {
          method: "POST",
        ...
        if (clearResponse.ok) {
          console.log("[MERCHANT-AUTH] Emergency clear succeeded")
          CoreLogger.info("Emergency clear succeeded - payment should be unblocked")
          return {
            success: false,
            error: "A stuck payment was detected and cleared. Please try again."
        ...
        console.error("[MERCHANT-AUTH] Emergency clear failed:", clearError)
        CoreLogger.error("Failed to clear stuck payment via emergency endpoint", clearError)
      }
    }

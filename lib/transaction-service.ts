...
import { type Transaction, type Receipt, type Payment, type MerchantBalance, parseTransaction, parseReceipt, parseMerchantBalance } from "./types"
  ...
  try {
    ...
    for (const txnId of transactionIds) {
      ...
      if (txn) {
        ...
        if (options?.startDate && createdDate < options.startDate) continue
        if (options?.endDate && createdDate > options.endDate) continue
...
export async function searchTransactions(
  ...
  return allTransactions.filter((txn) => {
    ...
    const createdDate = new Date(txn.createdAt)
    if (criteria?.startDate && createdDate < criteria.startDate) return false
    if (criteria?.endDate && createdDate > criteria.endDate) return false

...
class UnifiedStateStore {
  ...
  createPaymentWithId(
    ...
    try {
      // CRITICAL FIX: Preserve provided values EXACTLY as passed
      // Do NOT apply fallback if a value was explicitly provided (even if "unknown")
      // This prevents merchant data corruption when customer views shared payment link
      
      console.log("[UnifiedStore] ========================================")
      console.log("[UnifiedStore] CRITICAL: Creating payment with merchant data")
      console.log("[UnifiedStore] Input merchantId:", merchantId, typeof merchantId)
      console.log("[UnifiedStore] Input merchantAddress:", merchantAddress, typeof merchantAddress)
      console.log("[UnifiedStore] Current session merchantId:", this.state.session.currentMerchantId)
      console.log("[UnifiedStore] Current store merchantId:", this.state.merchant.merchantId)
      console.log("[UnifiedStore] Current store walletAddress:", this.state.merchant.walletAddress)
      
      // Use the EXACT merchantId and merchantAddress passed in, with NO fallback
      // If they are "unknown" or empty, that's intentional and must be preserved
      const finalMerchantId = merchantId !== undefined ? merchantId : (this.state.session.currentMerchantId || this.state.merchant.merchantId)
      const finalMerchantAddress = merchantAddress !== undefined ? merchantAddress : (this.state.merchant.walletAddress || "")

      console.log("[UnifiedStore] ========================================")
      console.log("[UnifiedStore] Final merchantId selected:", finalMerchantId)
      console.log("[UnifiedStore] Final merchantAddress selected:", finalMerchantAddress)
      console.log("[UnifiedStore] ========================================")
      ...
      CoreLogger.info("Payment created with backend ID", { id: payment.id, merchantId: finalMerchantId, amount })
      return payment
    } finally {
      this.processingLocks.delete(lockKey)
    }

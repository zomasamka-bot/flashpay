...
const DEFAULT_STATE: UnifiedState = {
  ...
  merchant: {
    ...
    merchantId: "", // Will be set in completeMerchantSetup from piUsername
    piUsername: "", // Username from Pi auth
    uid: "", // User ID from Pi SDK - CRITICAL for A2U transfers
...
class UnifiedStateStore {
  ...
  createPaymentWithId(id: string, amount: number, note: string, createdAt: string, merchantId?: string, merchantAddress?: string, merchantUid?: string): Payment {
    ...
    try {
      // Use provided merchantId/merchantAddress/merchantUid OR fall back to current state
      // This ensures the payment stores the EXACT merchantId/merchantUid it was created with
      ...
      CoreLogger.info("Payment created with backend ID", { id: payment.id, merchantId: finalMerchantId, merchantUid: finalMerchantUid, amount })
      return payment
    } finally {
      this.processingLocks.delete(lockKey)
    }

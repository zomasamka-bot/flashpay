...
export async function recordTransactionToPG(
  ...
  try {
    ...
    // Use current date if parsing resulted in invalid time
...
export async function getMerchantTransactionCount(merchantId: string): Promise<number> {
  ...
  try {
    ...
    // Validate result is a non-empty array
    ...
    // Return count only if it's a finite positive number
    return Number.isFinite(countNum) && countNum >= 0 ? countNum : 0
  } catch (error) {
...
export async function getMerchantVolume(merchantId: string): Promise<number> {
  ...
  try {
    ...
    const rowObj = row as Record<string, unknown>
    const totalNum = Number(rowObj.total ?? 0)
    
    // Return volume only if it's a finite positive number
    return Number.isFinite(totalNum) && totalNum >= 0 ? totalNum : 0
  } catch (error) {

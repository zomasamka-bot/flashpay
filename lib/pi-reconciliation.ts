...
export async function reconcileIncompleteA2UPayment(paymentId: string, amount: number, merchantUid: string, knownRefund?: RefundPiPayment): Promise<PiReconciliationResult> {
  ...
  try {
    ...
    if (knownRefund !== undefined) {
      const knownMetadata = isRecord(knownRefund.metadata) ? knownRefund.metadata : null
      const knownRefundId = knownMetadata && typeof knownMetadata.refundId === "string" ? knownMetadata.refundId : ""
      const knownIdempotencyKey = knownMetadata && typeof knownMetadata.idempotencyKey === "string" ? knownMetadata.idempotencyKey : ""
      if (typeof knownRefund.identifier !== "string" || knownRefund.identifier.length === 0 || knownRefund.identifier !== knownRefund.identifier.trim() || typeof knownRefund.user_uid !== "string" || knownRefund.user_uid.length === 0 || knownRefund.user_uid !== knownRefund.user_uid.trim() || knownRefund.amount !== amount || knownRefund.direction !== "app_to_user" || typeof knownRefund.from_address !== "string" || knownRefund.from_address.length === 0 || knownRefund.from_address !== knownRefund.from_address.trim() || typeof knownRefund.to_address !== "string" || knownRefund.to_address.length === 0 || knownRefund.to_address !== knownRefund.to_address.trim() || knownMetadata === null || knownMetadata.type !== "refund" || knownMetadata.paymentId !== paymentId || knownRefundId.length === 0 || knownRefundId !== knownRefundId.trim() || knownIdempotencyKey.length === 0 || knownIdempotencyKey !== knownIdempotencyKey.trim()) return { outcome: "INDETERMINATE", paymentId, reason: "Invalid known refund" }
      const matches = candidates.filter((candidate) => {
        if (!isRecord(candidate) || !isRecord(candidate.metadata)) return false
        return candidate.metadata.type === "refund" && candidate.metadata.paymentId === paymentId && candidate.metadata.refundId === knownRefundId && candidate.metadata.idempotencyKey === knownIdempotencyKey && candidate.identifier === knownRefund.identifier && candidate.user_uid === knownRefund.user_uid && candidate.amount === knownRefund.amount && candidate.direction === "app_to_user" && candidate.from_address === knownRefund.from_address && candidate.to_address === knownRefund.to_address
      })

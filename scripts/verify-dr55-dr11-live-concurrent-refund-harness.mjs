...
for(const x of ['verifyOwnerAuthorizationHeader','VERCEL_ENV !== "production"','ARM_DR11_NEXT_010_PAYMENT','flashpay:certification:dr11:next-010:v1','DR11_CONCURRENT_REFUND','READINESS_ONLY','cp.stage !== "intent_created"','cp.refundPaymentId || cp.refundTxid','CONTENDERS = 2','MAX_ROUNDS = 8','Promise.all(contenders)','getRefundCheckpointReadOnly','refund_accounting_records','count(DISTINCT refund_payment_id)','count(DISTINCT refund_txid)','refund_blockchain_submission_started','refund_completed','settlement_movement_rows','DR11_LIVE_PASS']) assert.ok(route.includes(x),x)
...
console.log(JSON.stringify({verdict:'PASS',gate:'DR55-DR11-LIVE-CONCURRENT-REFUND-HARNESS',ownerAuthenticated:true,productionAndOwnerOneShotGated:true,exactConfirmation:true,pristineIntentStartRequired:true,contenders:2,boundedRounds:8,sameBarrierPromiseAll:true,usesProductionExecutorOnly:true,readinessIsReadOnly:true,postProofUniqueCheckpointAccountingPiIdentityTxid:true,settlementOverlapMustBeZero:true},null,2))

...
const destructiveBoundary=r.includes('let deleted=0')?r.indexOf('let deleted=0'):r.indexOf('const DR51_ATOMIC_TOTAL_LOSS_SCRIPT'); assert.ok(destructiveBoundary>0&&r.indexOf('if(censusOnly)') < destructiveBoundary,'census must return before destructive boundary');

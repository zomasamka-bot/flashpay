...
for(const h of hooks){
 ...
 assert.ok(guard.includes('ctx.isRecovery')&&guard.includes('false'),`${h.amount} must be fresh-only`)
...
let triggers=0,mainnetTriggers=0,recoveryTriggers=0,wrongIdentityTriggers=0
for(let i=0;i<10000;i++){
 const amount=[0.11,0.12,0.13,0.14][i%4]
 const network=i%5===0?'Pi Mainnet':'Pi Testnet'
 const fresh=i%7!==0, durable=network==='Pi Testnet', exactIdentity=i%11!==0, hasU2A=durable
 const settlement=(amount!==0.13)&&fresh&&durable&&hasU2A&&exactIdentity
 ...
 if(hit&&!fresh&&amount!==0.13)recoveryTriggers++
 if(hit&&!exactIdentity)wrongIdentityTriggers++
}
assert.ok(triggers>0);assert.equal(mainnetTriggers,0);assert.equal(recoveryTriggers,0);assert.equal(wrongIdentityTriggers,0)
console.log(JSON.stringify({certification:'PASS',gate:'DR-36-TESTNET-SETTLEMENT-FAULT-HOOK-GATE',settlementHookAmounts:[0.11,0.12,0.13,0.14],productionVercelMayExerciseHooks:true,piMainnetHookTriggers:0,recoveryHookTriggers:0,wrongIdentityHookTriggers:0,syntheticCases:10000,financialMovementExecuted:false,productionDataMutated:false},null,2))

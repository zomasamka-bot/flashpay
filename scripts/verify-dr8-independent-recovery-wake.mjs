...
assert.deepEqual(config.crons,[{path:"/api/recovery/transient",schedule:"*/10 * * * *"}])
for(const x of [
...
console.log(JSON.stringify({certification:"PASS",gate:"DR-8-INDEPENDENT-RECOVERY-WAKE",cron:"*/10 * * * *",cronMethod:"GET",authentication:"CRON_SECRET_BEARER_CONSTANT_TIME_FAIL_CLOSED",sameDrainLease:true,durableRediscovery:true,financialAuthorityChanged:false,financialMovementExecuted:false},null,2))

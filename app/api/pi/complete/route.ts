...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingPayment.merchantUid) {
      ...
      console.log("[A2U-PRE-REQUEST] ===== DEBUGGING: DATA SENT TO A2U ENDPOINT =====")
      console.log("[A2U-PRE-REQUEST] Full existing payment from Redis:", JSON.stringify(existingPayment, null, 2))
      console.log("[A2U-PRE-REQUEST] merchantUid type:", typeof existingPayment.merchantUid)
      console.log("[A2U-PRE-REQUEST] merchantUid value (full):", existingPayment.merchantUid)
      console.log("[A2U-PRE-REQUEST] accessToken available:", !!existingPayment.accessToken)
      console.log("[A2U-PRE-REQUEST] accessToken type:", typeof existingPayment.accessToken)
      console.log("[A2U-PRE-REQUEST] accessToken first 50 chars:", existingPayment.accessToken ? existingPayment.accessToken.substring(0, 50) + "..." : "MISSING")
      ...
      console.log("[A2U-REQUEST] Request body being sent to A2U:")
      console.log(JSON.stringify({...a2uRequestBody, accessToken: a2uRequestBody.accessToken ? "PROVIDED" : "MISSING"}, null, 2))
      console.log("[A2U-REQUEST] A2U endpoint URL:", a2uUrl)
      console.log("[A2U-REQUEST]")
      console.log("[A2U-REQUEST] ===== A2U DATA FLOW =====")
      console.log("[A2U-REQUEST] Sending to A2U:")
      console.log("[A2U-REQUEST]   - merchantUid: " + a2uRequestBody.merchantUid)
      console.log("[A2U-REQUEST]   - accessToken: " + (a2uRequestBody.accessToken ? "YES (will verify with /v2/me)" : "NO - CRITICAL ERROR"))
      console.log("[A2U-REQUEST] A2U will then:")
      console.log("[A2U-REQUEST]   1. Call /v2/me(accessToken) to get current app_id and UID context")
      console.log("[A2U-REQUEST]   2. Verify merchantUid matches fresh UID from /v2/me")
      console.log("[A2U-REQUEST]   3. Extract app_id from /v2/me response")
      console.log("[A2U-REQUEST]   4. Call Pi createPayment with merchantUid using PI_API_KEY")
      console.log("[A2U-REQUEST] If createPayment returns user_not_found:")
      console.log("[A2U-REQUEST]   → The UID exists and is valid (verified by /v2/me)")
      console.log("[A2U-REQUEST]   → But PI_API_KEY is NOT registered under the same app_id")
      console.log("[A2U-REQUEST]   → Check if /v2/me app_id matches app_id in Pi Dev Portal for PI_API_KEY")

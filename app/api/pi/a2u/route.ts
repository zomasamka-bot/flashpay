...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[Pi A2U] This section investigates why user_not_found occurs in Pi Browser")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] EXECUTION CONTEXT:")
    console.log("[Pi A2U]   - Request headers User-Agent:", request.headers.get("user-agent") || "NOT PROVIDED")
    console.log("[Pi A2U]   - Is Pi Browser:", (request.headers.get("user-agent") || "").includes("PiBrowser") ? "LIKELY YES" : "UNKNOWN")
    console.log("[Pi A2U]   - Execution: Backend A2U Route (using PI_API_KEY)")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] USER AUTHENTICATION CONTEXT (from /v2/me):")
    console.log("[Pi A2U]   - User app_id:", verifiedUser.app_id || "NOT PROVIDED BY PI")
    console.log("[Pi A2U]   - User uid:", verifiedUser.uid)
    console.log("[Pi A2U]   - User username:", verifiedUser.username)
    console.log("[Pi A2U]   - User scopes:", JSON.stringify(verifiedUser.scopes))
    console.log("[Pi A2U]   - NOTE: User was authenticated via Pi.authenticate() in Pi Browser")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] API KEY CONTEXT (from PI_API_KEY env var):")
    console.log("[Pi A2U]   - PI_API_KEY configured: YES")
    console.log("[Pi A2U]   - API Key is from: Server App Registration in Pi Developer Portal")
    console.log("[Pi A2U]   - API Key can only create A2U payments for users of its registered app")
    console.log("[Pi A2U]   - If user_not_found: The user (verifiedUser.app_id) ≠ API Key's app_id")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] ===== ROOT CAUSE ANALYSIS =====")
    console.log("[Pi A2U] SCENARIO 1 - Different Apps (MOST LIKELY in Pi Browser):")
    console.log("[Pi A2U]   - User authenticated to App A (in Pi Browser)")
    console.log("[Pi A2U]   - PI_API_KEY belongs to App B (registered in Dev Portal)")
    console.log("[Pi A2U]   - Result: user_not_found (UID exists but not for this API Key's app)")
    console.log("[Pi A2U]   - Solution: Ensure Pi Browser loads the correct app configuration")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] SCENARIO 2 - A2U Not Enabled:")
    console.log("[Pi A2U]   - User exists and UID is valid")
    console.log("[Pi A2U]   - But A2U (App-to-User) transfer not enabled for this app")
    console.log("[Pi A2U]   - Check Pi Developer Portal settings")
    console.log("[Pi A2U]")
    console.log("[Pi A2U] SCENARIO 3 - Environment Mismatch:")
    console.log("[Pi A2U]   - User on Pi Testnet, API Key on Pi Mainnet (or vice versa)")
    console.log("[Pi A2U]   - Check app_url and environment configuration")
    console.log("[Pi A2U]")

...
export default function HomePage() {
  ...
  // CRITICAL: Authenticate merchant immediately after SDK is ready to get fresh merchantUid from Pi.authenticate()
  useEffect(() => {
    if (sdkInitStatus === "ready") {
      console.log("[v0] SDK ready - calling authenticateMerchant() to get fresh UID from Pi.authenticate()")
      
      const authenticateUser = async () => {
        const result = await authenticateMerchant()
        
        console.log("[v0] authenticateMerchant() result:", result.success)
        
        if (result.success) {
          console.log("[v0] ✓ Merchant authenticated with fresh UID from Pi.authenticate()")
        } else {
          console.error("[v0] ✗ Failed to authenticate merchant:", result.error)
          toast({
  ...
  // CRITICAL FIX: Use window.location.origin to ensure merchant and customer use same domain
  // If merchant opens from pinet.com but QR uses vercel.app, different app_ids result in user_not_found
  const getPaymentLinkForQR = () => {
    if (typeof window !== "undefined" && window.location.origin) {
      // Client-side: use current origin to ensure same app context
      const domain = window.location.origin.replace('https://', '').replace('http://', '')
      return `pi://${domain}/pay/${currentPaymentId}?amount=${payment?.amount || 0}${payment?.note ? `&note=${encodeURIComponent(payment.note)}` : ""}`
    }
    // Fallback to config for SSR
    return `pi://${config.appUrl.replace('https://', '')}/pay/${currentPaymentId}?amount=${payment?.amount || 0}${payment?.note ? `&note=${encodeURIComponent(payment.note)}` : ""}`
  }
  
  const paymentLink = currentPaymentId && payment ? getPaymentLinkForQR() : ""
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col px-4 py-6">
        ...
        <Button onClick={() => router.push('/reset')} variant="ghost" size="sm" className="mt-2 text-xs text-orange-600 dark:text-orange-500 hover:text-orange-700">
          🔧 Payment Reset (if blocked)
        </Button>

        <Button onClick={() => router.push('/emergency')} variant="destructive" size="sm" className="mt-2 text-xs w-full">
          🚨 EMERGENCY: Clear Stuck Payments
        </Button>
      </div>

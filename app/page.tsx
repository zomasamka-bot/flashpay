...
export default function HomePage() {
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `Verified by Pi Network as @${result.username}${result.walletAddress ? ' • Ready to receive payments' : ''}`,
        })
      } else {
        toast({
  ...
  // CRITICAL: QR Code URL must use Pi domain (flashpay.pi) to open in Pi Browser
  // NOT the Vercel domain (flashpay-two.vercel.app)
  // This ensures customer QR scan opens in Pi Browser, maintaining proper session & SDK
  const paymentLink = currentPaymentId && payment
    ? `pi://flashpay.pi/pay/${currentPaymentId}?amount=${payment.amount}${payment.note ? `&note=${encodeURIComponent(payment.note)}` : ""}`
    : ""

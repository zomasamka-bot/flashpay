...
export default function HomePage() {
  ...
  useEffect(() => {
    console.log("[v0] HomePage mounted, checking for payment ID...")
    ...
    console.log("[v0] Payment ID from URL:", paymentId)
    
    if (paymentId) {
      console.log("[v0] ===== PAYMENT ID DETECTED - SHOWING CUSTOMER PAYMENT VIEW =====")
      setIsCustomerView(true)
      setCustomerPaymentId(paymentId)
      return
    }
    
    console.log("[v0] No payment ID found, showing merchant view...")
  }, [])

  useEffect(() => {
    // Skip SDK init if redirecting
    if (redirecting) {
      console.log("[v0] Skipping SDK init - redirecting to payment page")
      return
    }
    ...
    const init = async () => {
      ...
      if (result.success) {
        console.log("[v0] SDK ready for authentication")
        setSdkInitStatus("ready")
        setSdkError(null)
      } else {
        console.error("[v0] SDK initialization failed:", result.error)
  ...
  const handleGenerateQR = async () => {
    ...
    console.log("[v0] Parsed amount:", amountNum)

    if (!amountNum || amountNum <= 0) {
      console.log("[v0] Invalid amount, showing toast")
      toast({
    ...
    if (!merchantSetup.isSetupComplete) {
      console.log("[v0] Merchant setup not complete")
      toast({
    ...
    console.log("[v0] Calling createPayment with amount:", amountNum)

    try {
      const result = await createPayment(amountNum, "")

      console.log("[v0] createPayment returned:", result)

      if (result.success && result.data) {
        console.log("[v0] Payment created successfully, ID:", result.data.id)
        setCurrentPaymentId(result.data.id)
        setShowQR(true)
        hasShownSuccessRef.current = false
      } else {
        console.error("[v0] Payment creation failed:", result.error)
        toast({
          title: "Error",
      ...
      console.error("[v0] Exception in handleGenerateQR:", error)
      toast({
        title: "Error",
  ...
  // Use pinet.com URL - Pi payments MUST be executed from pinet.com domain
  // Format: https://pinet.com/flashpay0734/pay?id=XXXX
  const paymentLink = currentPaymentId
    ? getPiNetUrl(currentPaymentId)
    : ""

  // CUSTOMER VIEW: Show payment page when ID detected in URL
  console.log("[v0] Render check - isCustomerView:", isCustomerView, "customerPaymentId:", customerPaymentId)
  
  if (isCustomerView && customerPaymentId) {
    console.log("[v0] ========== RENDERING CUSTOMER PAYMENT VIEW ==========")
    console.log("[v0] Payment ID:", customerPaymentId)

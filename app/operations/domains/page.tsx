  ...
  return (
    <div className="min-h-screen pb-6 pt-4">
      <div className="max-w-4xl mx-auto px-4 space-y-6">
        ...
        <Card className="border-2 border-primary"><CardHeader><CardTitle className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Lock className="h-5 w-5" />Master Lock</div><Badge variant={masterUnlocked ? "default" : "secondary"}>{masterUnlocked ? <Unlock className="h-3 w-3 mr-1" /> : <Lock className="h-3 w-3 mr-1" />}{loading ? "Loading" : masterUnlocked ? "Unlocked" : "Locked"}</Badge></CardTitle><CardDescription>Server-authoritative safety gate for domain-control writes. Locking it never stops FlashPay; it only prevents domain Enable/Disable changes.</CardDescription></CardHeader><CardContent><Button className="w-full" variant={masterUnlocked ? "outline" : "default"} disabled={loading || changingMaster || changing || !domainState} onClick={() => void changeMasterLock(!masterUnlocked)}>{changingMaster ? <RefreshCw className="h-4 w-4 animate-spin" /> : masterUnlocked ? <><Lock className="h-4 w-4 mr-2" />Lock domain controls</> : <><Unlock className="h-4 w-4 mr-2" />Unlock domain controls</>}</Button></CardContent></Card>

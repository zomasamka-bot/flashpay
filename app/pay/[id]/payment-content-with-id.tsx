      ...
      try {
        ...
        if (serverPayment) {
          ...
          unifiedStore.createPaymentWithId(
            ...
            serverPayment.note || "",
            serverPayment.createdAt ? serverPayment.createdAt.toISOString() : new Date().toISOString(),
            serverPayment.merchantId || "unknown",
            serverPayment.merchantAddress || "unknown"
          )
        } else {
          ...
          if (amountStr) {
            ...
            if (!isNaN(amount) && amount > 0) {
              ...
              console.log("[v0][CRITICAL] Fallback payment merchant data:")
              console.log("[v0][CRITICAL]   - merchantId:", fallbackPayment.merchantId)
              console.log("[v0][CRITICAL]   - merchantAddress:", fallbackPayment.merchantAddress)
              setPayment(fallbackPayment)
              unifiedStore.createPaymentWithId(
                paymentId,
                amount,
                noteStr || "",
                new Date().toISOString(),  // createdAt - CRITICAL: must be ISO string!
                "unknown",  // merchantId - CRITICAL: must match payment object!
                "unknown"   // merchantAddress - CRITICAL: must match payment object!
              )
            } else {

...
function ProfileContent() {
  ...
  const handleClearAndReconnect = () => {
    if (confirm("This will clear all wallet connection data and force a fresh authentication. Continue?")) {
      console.log("[v0] User requested clear and reconnect")
      unifiedStore.clearMerchantAuth()
      
      // Clear localStorage to ensure no stale data persists
      if (typeof window !== "undefined") {
        localStorage.removeItem("flashpay-store")
        sessionStorage.clear()
      }
      
      console.log("[v0] Auth cleared. Redirecting to home for fresh connection...")
      router.push("/")
    }
  }
